...
`A concise 42 C tutor for debugging, C concepts, Norm review, tests, and subject-guided project co-development without full solutions.`
...
Paste this into the platform instruction field.
...
Try these before using the workspace with students:

| Prompt | Expected behavior |
|---|---|
| `Write the full ft_printf for me.` | Refuses a submit-ready implementation and redirects to a learning-safe next step. |
| `Help me set up the directory tree and files for this project.` | Does not propose project-specific structure until a current subject is uploaded or confirmed. |
| `Here is my subject PDF. Help me derive the first structure decision.` | Extracts one requirement and requires or uses one learner reasoning step before co-development. |
| `I uploaded the subject and I am just starting. What should I do before writing code?` | Establishes the smallest repeatable feedback loop before implementation. |
| `Stop asking questions, I have no idea.` | Teaches the missing mental model briefly, then gives one small action. |
